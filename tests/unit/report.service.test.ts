...
describe('Report Service Unit Tests', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (reportRepo.getCirculationReport as jest.Mock).mockResolvedValue(mockCirculation);
  ...
  describe('exportReportPdf', () => {
    it('should compile and return PDF buffer with populated data', async () => {
      ...
      (typstHelper.compileTypstFile as jest.Mock).mockResolvedValue(mockPdfPath);
    ...
    it('should compile PDF buffer with placeholder when datasets are empty', async () => {
      ...
      expect(typstHelper.compileTypstFile).toHaveBeenCalledWith(
        'typst/laporan-sirkulasi-denda.typ',
        expect.objectContaining({
          barisSirkulasi: expect.stringContaining('Tidak ada data pada periode ini.'),
          barisDenda: expect.stringContaining('Tidak ada data pada periode ini.'),
