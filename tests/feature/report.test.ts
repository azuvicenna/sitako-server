...
jest.mock('@/controllers/librarian/report.controller', () => ({
  getCirculationReportHandler: jest.fn((req, res) => {
    ...
    if (format === 'csv') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="laporan-sirkulasi.csv"');
    ...
    if (format === 'xlsx') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="laporan-sirkulasi.xlsx"');
      return res.status(200).send(Buffer.from('mock-xlsx'));
    }
    if (format === 'pdf') {
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', 'attachment; filename="laporan-sirkulasi-denda.pdf"');
  ...
  getFineReportHandler: jest.fn((req, res) => {
    ...
    if (format === 'csv') {
      res.setHeader('Content-Type', 'text/csv; charset=utf-8');
      res.setHeader('Content-Disposition', 'attachment; filename="laporan-denda.csv"');
    ...
    if (format === 'xlsx') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="laporan-denda.xlsx"');
      return res.status(200).send(Buffer.from('mock-xlsx'));
    }
    if (format === 'pdf') {
      res.setHeader('Content-Type', 'application/pdf');
...
describe('Report Endpoints (Librarian)', () => {
  ...
  describe('GET /api/reports/circulation', () => {
    ...
    it('should return 400 when startDate is greater than endDate', async () => {
      const res = await request(app)
        .get('/api/reports/circulation?startDate=2026-09-15&endDate=2026-09-01')
        .set('Cookie', `token=${token}`);
    ...
    it('should return 200 with json data by default', async () => {
      const res = await request(app)
        .get('/api/reports/circulation?startDate=2026-09-01&endDate=2026-09-14')
        .set('Cookie', `token=${token}`);
    ...
    it('should return 200 with csv format', async () => {
      ...
      expect(res.headers['content-disposition']).toContain('filename="laporan-sirkulasi.csv"');
    });
    ...
    it('should return 200 with xlsx format', async () => {
      ...
      expect(res.headers['content-disposition']).toContain('filename="laporan-sirkulasi.xlsx"');
    });
