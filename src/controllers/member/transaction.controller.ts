...
export const showMyTransaction = async (req: Request, res: Response) => {
  try {
    ...
    const result = await transactionService.getTransactionById(transactionId, memberId);
...
export const createMyTransaction = async (req: Request, res: Response) => {
  try {
    ...
    const validationResult = await validateTransactionCreation(memberId, body.bukuId);

    if (!validationResult.success) {
      return sendFail(res, 400, validationResult.message ?? 'Validasi transaksi gagal');
    }

    const result = await transactionService.createNewTransaction(memberId, body);
...
export const returnMyTransaction = async (req: Request, res: Response) => {
  try {
    ...
    const result = await kembalikanBuku(transactionId, memberId, Boolean(isBukuHilang));
