...
const isValidStatusActive = (status: unknown): status is StatusActive => {
  return typeof status === 'string' && (VALID_STATUS_ACTIVE as readonly string[]).includes(status);
};
...
export const getMemberHandler = async (req: Request, res: Response) => {
  try {
    ...
    const result = await memberService.getMembersWithPagination(statusActive, page, limit, search);
...
export const createMember = async (req: Request, res: Response) => {
  try {
    ...
    const result = await memberService.createNewMember(req.body ?? {}, validatedFile);
