...
export const getCirculationReportHandler = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (format === "csv") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="laporan-sirkulasi-${today}.csv"`,
      );
      return res.send(buffer);
    }
...
export const getFineReportHandler = async (req: Request, res: Response) => {
  try {
    ...
    if (format === "xlsx") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="laporan-denda-${today}.xlsx"`,
      );
      return res.send(buffer);
    }
    ...
    if (format === "csv") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="laporan-denda-${today}.csv"`,
      );
      return res.send(buffer);
    }
