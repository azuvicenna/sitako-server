...
export const validateTransactionCreation = async (
  ...
  const penaltyTransactions = await db
    ...
    .leftJoin(
      finePayments,
      and(eq(finePayments.transaksiId, transactions.id), eq(finePayments.paymentStatus, 'PAID')),
    )
    .where(
      and(
  ...
  const lateTransaction = penaltyTransactions.find((trx) => trx.status === 'Terlambat');
  if (lateTransaction) {
    return {
      success: false,
  ...
  const [bookResult, borrowedResult, activeTransactionsResult] = await Promise.all([
    db.select({ stock: books.jumlahStok }).from(books).where(eq(books.id, bookId)).limit(1),
    db
    ...
    db
      ...
      .where(
        and(
          eq(transactions.anggotaId, memberId),
          inArray(transactions.status, ['Menunggu Persetujuan', 'Menunggu Diambil', 'Dipinjam']),
        ),
      ),
  ]);
  ...
  const activeTransactionsCount = Number(activeTransactionsResult[0]?.count ?? 0);
