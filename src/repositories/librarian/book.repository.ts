...
const clearBookCache = async (bookType?: BookSelect['tipeBuku']) => {
  ...
  await Promise.all([clearCacheByPattern([pattern, 'member:books:*']), invalidateDashboardCache()]);
};
...
export const removeBookById = async (id: string): Promise<BookSelect | null> => {
