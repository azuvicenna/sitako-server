...
const clearMemberCache = async () => {
  await Promise.all([clearCacheByPattern('member:*'), invalidateDashboardCache()]);
};
...
export const findMemberRawById = async (id: string): Promise<MemberSelect | null> => {
...
export const removeMemberById = async (id: string): Promise<MemberSelect | null> => {
