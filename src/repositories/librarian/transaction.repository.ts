...
const clearTransactionCache = async () => {
  await Promise.all([clearCacheByPattern('transaction:*'), invalidateDashboardCache()]);
};
...
export const findTransaction = async (id: string): Promise<TransactionSelect | null> => {
...
export const removeTransactionById = async (id: string): Promise<TransactionSelect | null> => {
