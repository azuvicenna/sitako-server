...
export const findLibrarianRawById = async (id: string): Promise<LibrarianSelect | null> => {
  const [librarian] = await db.select().from(librarians).where(eq(librarians.id, id)).limit(1);
...
export const updateLibrarianById = async (
  ...
  const [updated] = await db.update(librarians).set(data).where(eq(librarians.id, id)).returning();
...
export const removeLibrarianById = async (id: string): Promise<LibrarianSelect | null> => {
